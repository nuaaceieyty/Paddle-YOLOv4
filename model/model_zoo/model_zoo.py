...
from model.core.workspace import load_config, create
from model.utils.checkpoint import load_weight
from model.utils.download import get_config_path
...
def list_model(filters=[]):
    model_zoo_file = pkg_resources.resource_filename('model.model_zoo',
                                                     MODEL_ZOO_FILENAME)
...
def get_config_file(model_name):
    return get_config_path("model://configs/{}.yml".format(model_name))


def get_weights_url(model_name):
    return "model://models/{}.pdparams".format(osp.split(model_name)[-1])
